!##########################################################
!
!       AmoebaFF Subroutine of minimization
!
!##########################################################
SUBROUTINE AMOEBAFF(P,Y,MP,NP,NDIM,FTOL,ITER)                  
...
!  IF(ITER.EQ.ITMAX) Pause ' AmoebaFFFF exceeding maximum iterations.'
  ...
  END DO
  YPR=fittingSpectrumFunction(PR)
  IF(YPR.LE.Y(ILO)) THEN
    DO J=1,NDIM
      PRR(J)=GAMMA*PR(J) + (1.d0-GAMMA)*PBAR(J)
    END DO
    YPRR=fittingSpectrumFunction(PRR)
  ...
  ELSE IF(YPR.GE.Y(INHI)) THEN
    ...
    ELSE
      ...
      P(I,J)=PR(J)
      END DO
          Y(I)=fittingSpectrumFunction(PR)
    END IF
      END DO
    END IF
  ELSE
