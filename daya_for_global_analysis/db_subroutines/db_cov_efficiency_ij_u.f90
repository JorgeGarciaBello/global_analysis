real(8) function db_cov_efficiency_ij_u(t13,dmee,i,j)
        ...
        select case(3)           
            case(3)
                do k=1,num_experiments
                    dat_i(k)=(db_S_f_i_Neff_u(i)-w_i*db_S_n_i_Neff_u(i))
                    dat_j(k)=(db_S_f_i_Neff_u(j)-w_j*db_S_n_i_Neff_u(j))
                enddo
                result_i=0.0d0
                result_j=0.0d0
                do k=1,num_experiments
                ...
                enddo
                db_cov_efficiency_ij_u=result/real(num_experiments)
        end select
    end if
    return
end function db_cov_efficiency_ij_u