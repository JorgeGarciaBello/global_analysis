!#################################################################################################################
!
!       expectedNumberOfNeutrinosByBinFar: is the function that return 
!           the expected neutirno spectrum bay energy-bin and by far-hall
!
!#################################################################################################################
real(8) function expectedNumberOfNeutrinosByBinFar(i)
    ...
    expectedNumberOfNeutrinosByBinFar=0.0d0
    h=(b-a)/real(n)
    do k=1,n
        x=a+h*real(k-1)
        expectedNumberOfNeutrinosByBinFar=expectedNumberOfNeutrinosByBinFar      &
                                    + h*( expectedNeutrinoSpectrum_H(hall,x)   &
                                         +expectedNeutrinoSpectrum_H(hall,x+h) &
                                        )/2.0d0
    enddo 
    expectedNumberOfNeutrinosByBinFar=ALPHA(i)*corrections_percetage(i)*expectedNumberOfNeutrinosByBinFar    
    return
end function expectedNumberOfNeutrinosByBinFar