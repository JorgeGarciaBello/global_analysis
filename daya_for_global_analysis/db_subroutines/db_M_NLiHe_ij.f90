real(8) function db_M_NLiHe_ij(t13,dmee,i,j)
    use db_data, only: num_experiments, N_obs_n, N_obs_f, rand_NLiHe_n, rand_NLiHe_f, rand_W_k
    implicit none
    real(8) :: t13, dmee    
    integer :: i,j
    ...
    do k=1,num_experiments
        Nbar_i(k)= N_obs_f(i)-(w_i*(N_obs_n(i)-rand_NLiHe_n(i,k)) + rand_NLiHe_f(i,k))
        Nbar_j(k)= N_obs_f(j)-(w_j*(N_obs_n(j)-rand_NLiHe_n(j,k)) + rand_NLiHe_f(j,k))
    enddo
    ...
    do k=1,num_experiments
        db_M_NLiHe_ij=db_M_NLiHe_ij+(Nbar_i(k)-Navg_i)*(Nbar_j(k)-Navg_j)
    enddo
    db_M_NLiHe_ij=db_M_NLiHe_ij/real(num_experiments)
    return
end function db_M_NLiHe_ij