subroutine db_create_M_stat_signal_bkg(t13,dmee,rand_W_k,Nbar_i,U,V)
    ...
    do i=1,NBIN*2*PD
        do j=1,NBIN*2*PD
            ...
            !V(i,j)=db_M_Total_ij(t13,dmee,i,j,rand_w_i,Nbar_i,U)
        enddo
    enddo    
    V=V+Vstat
    !call db_write_matrix(NBIN*2*PD,V,'M_stat_signal_bkg1')
    !call db_write_matrix_m_n(NBIN*2*PD,V,'M_M_stat_signal_bkg')
    call db_get_inverse_covariance_matrix(NBIN*2*PD,V)
    !call db_write_matrix(NBIN*2*PD,V,"M_inv_stat_signal_bkg")
    return
